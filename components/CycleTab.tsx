...
const CycleTab: React.FC<CycleTabProps> = ({ cycleItems, updateCycleItem, clearCycle, deleteCycleItem }) => {
  ...
  return (
    <div className="space-y-4">
      {/* Modal de Confirmação para Limpar Ciclo */}
      {showClearConfirm && (
              ...
              <div key={item.id} className={`group border-2 rounded-2xl p-4 flex items-center gap-4 transition-all ${item.completed ? 'bg-gray-50 border-transparent opacity-60' : 'bg-white border-gray-50 hover:border-indigo-100 shadow-sm'}`}>
                ...
                <div className="flex-grow">
                  <div className="flex items-center gap-3">
                    ...
                    {item.notebookUrl && item.notebookUrl.length > 5 && (
                      <a 
                        href={item.notebookUrl} 
                        target="_blank" 
                        rel="noopener noreferrer" 
                        className="bg-indigo-50 p-1.5 rounded-lg text-indigo-500 hover:bg-indigo-600 hover:text-white transition-all"
                        title="Ver Caderno"
                      >
                ...
                {/* MODAL DE REGISTRO - CORREÇÃO DE UI */}
                {editingId === item.id && (
                  <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4 modal-fade-in">
                    <div className="bg-white p-8 rounded-[40px] w-full max-w-sm shadow-2xl modal-zoom-in border border-gray-100">
                      <div className="mb-8">
                        <h3 className="text-xl font-black uppercase text-gray-800 tracking-tight text-center">Registrar Progresso</h3>
                        <p className="text-[10px] text-gray-400 font-bold uppercase text-center mt-1">{item.name}</p>
                      </div>
                      
                      <div className="space-y-4">
                        <div className="flex items-center gap-1 bg-gray-50 p-1.5 rounded-2xl mb-4">
                           <button onClick={() => setAddMode(true)} className={`flex-1 py-2 text-[10px] font-black rounded-xl uppercase transition-all ${addMode ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:bg-gray-100'}`}>Somar ao Total</button>
                           <button onClick={() => setAddMode(false)} className={`flex-1 py-2 text-[10px] font-black rounded-xl uppercase transition-all ${!addMode ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:bg-gray-100'}`}>Definir Valor</button>
                        </div>

                        <div className="space-y-4">
                          <div className="relative">
                            <label className="absolute -top-2.5 left-4 px-2 bg-white text-[10px] font-black text-green-600 uppercase tracking-widest">Acertos</label>
                            <input 
                              type="number" 
                              value={inputCorrect} 
                              onChange={e => setInputCorrect(Number(e.target.value))} 
                              className="w-full p-4 border-2 border-gray-100 rounded-2xl outline-none font-black text-lg text-gray-800 focus:border-green-400 transition-all" 
                            />
                          </div>
                          <div className="relative">
                            <label className="absolute -top-2.5 left-4 px-2 bg-white text-[10px] font-black text-red-600 uppercase tracking-widest">Erros</label>
                            <input 
                              type="number" 
                              value={inputWrong} 
                              onChange={e => setInputWrong(Number(e.target.value))} 
                              className="w-full p-4 border-2 border-gray-100 rounded-2xl outline-none font-black text-lg text-gray-800 focus:border-red-400 transition-all" 
                            />
                          </div>
                        </div>
                        ...
                        <div className="pt-2">
                           <input 
                             ...
                             className="w-full p-3 border-2 border-dashed border-gray-100 rounded-2xl outline-none font-bold text-xs text-gray-400 focus:border-indigo-200 transition-all" 
                           />
                        </div>
                      </div>

                      <div className="flex gap-4 mt-10">
                        <button onClick={() => setEditingId(null)} className="flex-1 py-4 font-black text-gray-400 hover:text-gray-600 uppercase text-xs transition-all">Cancelar</button>
                        <button onClick={() => saveEdit(item)} className="flex-1 bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-xl shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all">Salvar</button>
                      </div>
                    </div>
                  </div>
                )}
